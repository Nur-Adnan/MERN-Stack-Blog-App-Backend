...
const generateUploadURL = async (filePath) => {
  try {
    const date = new Date();
...
server.post("/upload-image", upload.single("image"), async (req, res) => {
  try {
    ...
    const imageUrl = await generateUploadURL(filePath);
    res.json({ imageUrl });
  } catch (error) {
...
// Endpoint to generate Cloudinary upload URL
server.get("/get-upload-url", async (req, res) => {
  try {
    const date = new Date();
    const imageName = `${nanoid()}-${date.getTime()}.jpeg`;

    const uploadURL = cloudinary.url(imageName, {
      folder: "bloggapp",
      resource_type: "auto",
      sign_url: true,
    });
